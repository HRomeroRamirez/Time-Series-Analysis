...
# The series goes from Q4-2008 to Q4-2025
...
# Scenario 1: Headline CPI Estimation -----------------------------------------
results_hcpi <- lp_lin(
...
# Scenario 2: Core CPI Estimation --------------------------------------------
results_ccpi <- lp_lin(
...
# Define the instrument (2-Year Expected Inflation (EXPINF2YR))
instrument_ie2 <- as.data.frame(macroData2[, "IE2 - US"])

# --- Scenario 1: Headline CPI & Unemployment ---
endog_hcpi <- as.data.frame(macroData2[, c("HCPI - PR", "UR - PR")]) # Define it!

results_iv_hcpi <- lp_lin_iv(
  endog_data      = endog_hcpi,
  lags_endog_lin  = 4,
  shock           = shock_pfr,      
  instrum         = instrument_ie2, 
  use_twosls      = TRUE,           
  trend           = 0,              
  hor             = 8,
  confint         = 1.96,
...
# --- Scenario 2: Core CPI & Unemployment ---
# This is the line that was likely missing or not run in your console:
endog_ccpi <- as.data.frame(macroData2[, c("CCPI - PR", "UR - PR")]) # Define it!
